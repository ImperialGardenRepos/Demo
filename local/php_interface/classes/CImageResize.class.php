...
class CImageResize
{
     ...
     * @param $imgId
     * @param $width int
     * @param int|string $height Если не задано, будет пропорционально ширине
     ...
     */
    public function getResizedImgOrPlaceholder($imgId, $width, $height = 'auto', $arParams = []): ?string
    {
        $proportional = $arParams['PROPORTIONAL'] ?? true;


        if (!$width) {
            throw new InvalidArgumentException('File dimensions can not be a null');
        }
        ...
        if (!$imgId) {
            ...
            $height = (int)$height === $autoHeightMax ? $width : $height;
        ...
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . $resizedImg['src'])) {
            if ((int)$height === $autoHeightMax) {
                $height = $width;
            }
            return static::getResizedImgOrPlaceholder(false, $width, $height, $proportional);
        }
