...
AddEventHandler("main", "OnProlog", array('\\ig\\CSystemEvents', "onPrologHandler"), 50);
AddEventHandler("main", "OnEndBufferContent", array('\\ig\\CSeo', "deleteKernelJs"));
AddEventHandler("main", "OnEndBufferContent", array('\\ig\\CSeo', "deleteKernelCss"));
AddEventHandler("main", "OnEndBufferContent", array('\\ig\\CSeo', "check404"));
...
class CSystemEvents {
	function onPrologHandler()
	{
		if(\Bitrix\Main\Context::getCurrent()->getRequest()->isAdminSection()) {
			...
			if(!$GLOBALS["SKIP_REDIRECT_CHECK"]) \ig\CRedirect::checkRedirect();
			
			\ig\CVirtualPage::checkVirtualPage();
