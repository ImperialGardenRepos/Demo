...
Main\Page\Asset::getInstance()->addJs('/local/js/admin/iblock_element_2_edit.js');
	...
	if (!$errorTriger)
	{
		...
		)
		{
			...
			else
			{
				...
				// заполненность полей
				$intCatalogID = \ig\CHelper::getFirst(
					$_REQUEST["PROP"][
					\ig\CHelper::getPropertyIDByCode("CML2_LINK", \ig\CHelper::getPropertyIDByCode('offers'))
					])["VALUE"];
				
				if($intCatalogID>0) {
					$intGroupID = \ig\CHelper::getGroupByCatalogID($intCatalogID);
					if($intGroupID>0) {
						foreach($arPropertyGroups as $strPropertyGroupCode => $arPropertyGroup) {
							if($arPropertyGroup["REQUIRED"] == 1) {
								if(in_array($intGroupID, $arPropertyGroup["LINKS"])) {
									$strPropertyValue = \ig\CHelper::getFirst($_REQUEST["PROP"][\ig\CHelper::getPropertyIDByCode($strPropertyGroupCode, \ig\CHelper::getPropertyIDByCode('catalog'))]);
				...
				$intPackPropertyID = \ig\CHelper::getPropertyIDByCode("PACK", \ig\CHelper::getPropertyIDByCode('offers'));
				$intPack = \ig\CHelper::getFirst(empty($_REQUEST["PROP"][$intPackPropertyID]["VALUE"])?$_REQUEST["PROP"][$intPackPropertyID]:$_REQUEST["PROP"][$intPackPropertyID]["VALUE"]);
				
				if($intPack == 60) {
					$intContainerPropertyID = \ig\CHelper::getPropertyIDByCode("CONTAINER", \ig\CHelper::getPropertyIDByCode('offers'));
					
					$intContainer = \ig\CHelper::getFirst(empty($_REQUEST["PROP"][$intContainerPropertyID]["VALUE"])?$_REQUEST["PROP"][$intContainerPropertyID]:$_REQUEST["PROP"][$intContainerPropertyID]["VALUE"]);
					if(empty($intContainer)) {
						$strWarning .= 'Свойство \'Контейнер (С)\' должно быть заполнено<br>';
					}
				}
				
//				$floatHeightNowFrom = (float) \ig\CHelper::getFirst($_REQUEST["PROP"][\ig\CHelper::getPropertyIDByCode("HEIGHT_NOW_FROM")]);
//				if($floatHeightNowFrom<0) $floatHeightNowFrom = $floatHeightNowFrom * (-1);
//
//				$floatHeightNowTo = (float) \ig\CHelper::getFirst($_REQUEST["PROP"][\ig\CHelper::getPropertyIDByCode("HEIGHT_NOW_TO")]);
//				if($floatHeightNowTo<0) $floatHeightNowTo = $floatHeightNowTo * (-1);
//
//				if($floatHeightNowFrom>0) {
//					if($floatHeightNowTo>0 && $floatHeightNowFrom > $floatHeightNowTo) {
//						$strWarning .= '"Высота сейчас, от" не может быть больше "Высота сейчас, до"<br>';
//					}
//				} else {
//					if($floatHeightNowTo>0) {
//						$strWarning .= 'Необходимо указать "Высота сейчас, от"<br>';
//					}
//				}
...
else
{
	...
	function skClearPropValue(&$PROP, $intPropID) {
		unset($PROP[$intPropID]["VALUE"]);
		unset($PROP[$intPropID]["~VALUE"]);
	}
	
	// чистим поля при копировании
	if($_REQUEST["action"] == 'copy' && !$bSave) {
		$arClearProp = array("HEIGHT_NOW", "STEM_GIRTH", "CROWN_WIDTH", "HEIGHT1", "HEIGHT2", "PACK", "AVAILABLE", "NEW", "ACTION", "AGES", "CONTAINER", "MULTISTEMMED", "CONTAINER_SIZE");
		foreach($arClearProp as $strCode) {
			skClearPropValue($PROP, \ig\CHelper::getPropertyIDByCode($strCode));
		}
		...
		if($arTranslit["TRANSLITERATION"] == "Y")
		{
			...
			<tr id="tr_NAME">
				...
				<td style="white-space: nowrap;">
					<input type="text" size="50" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><img id="name_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/bitrix/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" /><br>
					<a onclick="return obHelper.transliterate('NAME', 'CODE', 'ru2en');" href="#">Сформировать латинское название</a>
				</td>
			</tr>
			<?
		...
		else
		{
			...
			<tr id="tr_NAME">
				...
				<td style="white-space: nowrap;">
					...
					<a onclick="return obIblockElementEdit2.setOfferDefaultName(false);" href="#">Установить название по-умолчанию</a>
				</td>
			</tr>
			<?
			$tabControl->EndCustomField("NAME",
...
//			$tabControl->AddEditField("NAME", GetMessage("IBLOCK_FIELD_NAME").":", true, array("size" => 50, "maxlength" => 255, "id" => "NAME"), $str_NAME);
		...
		if(!empty($PROP)):
			...
			foreach($PROP as $prop_code=>$prop_fields):
				...
				?>
				<tr data-code="<?=$prop_fields["CODE"]?>" class="js-tr-<?=$prop_fields["CODE"]?><?=($prop_fields["PROPERTY_TYPE"]=="F"?' adm-detail-file-row':'')?><?=(in_array($prop_fields["CODE"], $arGroupAddictable)?' js-tr-group-addictable':'')?>" id="tr_PROPERTY_<?echo $prop_fields["ID"];?>">
					...
					<td width="60%"><?
						...
						if($prop_fields["MULTIPLE"] == 'N') {
							if(strpos($strPropHtml, 'id="') === false) {
