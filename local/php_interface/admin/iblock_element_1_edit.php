...
$strOriginalFilename = 'iblock_element_edit.php';
$strCurrentFile = str_replace(__DIR__.'/', '', __FILE__);
...
while($arPropertyGroup = $rsPropertyGroup -> Fetch()) {
	$arPropertyGroups[$arPropertyGroup["UF_CODE"]] = array(
		...
		"ID" => $arPropertyGroup["ID"],
		"LINKS" => $arPropertyGroup["UF_POINTER"],
		"REQUIRED" => $arPropertyGroup["UF_REQUIRED"]
	);
}
...
Main\Page\Asset::getInstance()->addJs('/local/js/helper.js');
Main\Page\Asset::getInstance()->addJs('/local/js/admin/iblock_element_1_edit.js');
$APPLICATION->SetAdditionalCSS("/local/css/admin/common_admin_styles.css");
$APPLICATION->SetAdditionalCSS("/local/css/admin/iblock_element_1_edit.css");
...
do{ //one iteration loop
	...
	if(!$USER -> IsAdmin()) {
		$tabControl->bShowSettings = false;
	}
	...
	if (!$errorTriger)
	{
		...
		)
		{
			if (CCatalogAdminTools::changeTabs($IBLOCK_ID, $ID, $arMainCatalog))
			{
				...
				$strRedirectUrl = $selfFolderUrl.CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, $arUrlParams, "&".$tabControl->ActiveTabParam());
				$strRedirectUrl = str_replace($strCurrentFile, $strOriginalFilename, $strRedirectUrl);
				
				$adminSidePanelHelper->localRedirect($strRedirectUrl);
		...
		{
			class cValidate {
				public static function getScalarPropertyValueFromPost($intPropertyID) {
					foreach($_POST["PROP"][$intPropertyID] as $intCnt => $strValue) {
						return $strValue;
					}
				}
			}
				...
				if(!$USER -> IsAdmin()) {
					...
					if($intGroupID>0) {
						foreach ($arPropertyGroups as $strPropertyGroupCode => $arPropertyGroup) {
							if($arPropertyGroup["REQUIRED"] == 1) {
								if(in_array($intGroupID, $arPropertyGroup["LINKS"])) {
									$strPropertyValue = \ig\CHelper::getFirst($_REQUEST["PROP"][\ig\CHelper::getPropertyIDByCode($strPropertyGroupCode, \ig\CHelper::getIblockIdByCode('catalog'))]);
									
									if(isset($strPropertyValue["VALUE"])) {
										$strPropertyValue = $strPropertyValue["VALUE"];
									}
									
									if(strlen($strPropertyValue) == 0) {
										$strWarning .= 'Свойство \''.$arPropertyGroup["NAME"].'\' должно быть заполнено<br>';
									}
								}
							}
						}
...
//				$floatHeight10From = (float) \ig\CHelper::getFirst($_REQUEST["PROP"][\ig\CHelper::getPropertyIDByCode("HEIGHT_10_FROM")]);
//				if($floatHeight10From<0) $floatHeight10From = $floatHeight10From * (-1);
//
//				$floatHeight10To = (float) \ig\CHelper::getFirst($_REQUEST["PROP"][\ig\CHelper::getPropertyIDByCode("HEIGHT_10_TO")]);
//				if($floatHeight10To<0) $floatHeight10To = $floatHeight10To * (-1);
//
//				if($floatHeight10From>0) {
//					if($floatHeight10To>0 && $floatHeight10From > $floatHeight10To) {
//						$strWarning .= '"Высота через 10 лет, от" не может быть больше "Высота через 10 лет, до"<br>';
//					}
//				} else {
//					if($floatHeight10To>0) {
//						$strWarning .= 'Необходимо указать "Высота через 10 лет, от"<br>';
//					}
//				}
				
				// кастомные обработки изменения контента
				$isRussianSort = is_array($_POST["PROP"][37]);
				
				$strRussianName = $_POST["NAME"];
				$strLatinName = \cValidate::getScalarPropertyValueFromPost(35);
				$strRussianSynonym = \cValidate::getScalarPropertyValueFromPost(34);
				$strLatinSynonym = \cValidate::getScalarPropertyValueFromPost(36);
				
				// привязка к разделам
				$intCnt = 0;
				$intCurrentSectionID = 0;
				foreach($_POST["IBLOCK_SECTION"] as $intSectionID) {
					if($intSectionID>0) {
				...
				if($intCnt !== 1) {
					$strWarning .= 'Сорт\вид должен быть привязан к одной ветке иерархии';
				}
				
				$intIsView = \ig\CHelper::getFirst($_REQUEST["PROP"][\ig\CHelper::getPropertyIDByCode("IS_VIEW")]);
				if(isset($intIsView["VALUE"])) $intIsView = $intIsView["VALUE"];
				
				if(empty($intIsView)) {
					if($isRussianSort) {
						if(strlen($strRussianName) == 0) {
							$strWarning .= 'Наименование по-русски не может быть пустым у русского сорта<br>';
						}
					} else {
						if(strlen($strLatinName) == 0) {
							$strWarning .= 'Наименование по-латински не может быть пустым у нерусского сорта<br>';
						}
					}
				} else {
					// достаем символьный код из парента
					$rsSec = CIBlockSection::GetList(false, array("ID" => $intCurrentSectionID, "IBLOCK_ID"=> \ig\CHelper::getIblockIdByCode('catalog')), false, array("CODE"));
					if($arSec = $rsSec -> Fetch()) {
						$_POST["CODE"] = $arSec["CODE"];
					}
				}
				...
				if(empty($strWarning)) {
					$arFilter = array(
						...
						"NAME" => $strRussianName,
						"SECTION_ID" => $intCurrentSectionID,
						"PROPERTY_NAME_LAT" => $strLatinName
					);
					
					if($ID>0) $arFilter["!ID"] = $ID;
					
					$rsI = CIBlockElement::GetList(Array(), $arFilter, false, array("nTopCount" => 1), array(
						"ID",
						"IBLOCK_ID"
					));
					if($arI = $rsI -> Fetch()) {
						$strWarning .= 'Элемент с таким названием уже есть (ID='.$arI["ID"].')';
					}
				}
				...
				if ($strWarning == '')
				{
					...
					// изменяем контент
					if($isRussianSort) {
						$strLatinNameTranslit = \ig\CUtil::translitText($strRussianName, 'ru2en');
						
						if(empty($strLatinName)) { // прописываем в латинское название
							foreach($arFields["PROPERTY_VALUES"][35] as $intCnt => &$strVal) {
								$strVal["VALUE"] = $strLatinNameTranslit;
								break;
							}
							unset($strVal);
						} else { // прописываем в латинские синонимы
							if($strLatinNameTranslit != $strLatinName) {
								if(strpos($strLatinSynonym, $strLatinNameTranslit) === false) {
									foreach ($arFields["PROPERTY_VALUES"][36] as $intCnt => &$strVal) {
										$strVal["VALUE"] .= (empty($strVal["VALUE"]) ? '' : ', ').$strLatinNameTranslit;
										break;
									}
									unset($strVal);
								}
						...
						$strRussianNameTranslit = \ig\CUtil::translitText($strLatinName, 'en2ru');
						
						if(empty($arFields["NAME"])) { // прописываем в русское название
							$arFields["NAME"] = $strRussianNameTranslit;
						} else { // прописываем в русские синонимы
							if($strRussianNameTranslit != $arFields["NAME"]) {
								if(strpos($strRussianSynonym, $strRussianNameTranslit) === false) {
									foreach ($arFields["PROPERTY_VALUES"][34] as $intCnt => &$strVal) {
										$strVal["VALUE"] .= (empty($strVal["VALUE"]) ? '' : ', ').$strRussianNameTranslit;
										break;
									}
									unset($strVal);
								}
...
//					}
//
//					$intHeightExtPropertyID = \ig\CHelper::getPropertyIDByCode("HEIGHT_10_EXT", \ig\CHelper::getPropertyIDByCode('catalog'));
//					$intHeightPropertyID = \ig\CHelper::getPropertyIDByCode("HEIGHT_10", \ig\CHelper::getPropertyIDByCode('catalog'));
//
//					$strNewHeightValue = '';
//					foreach($arFields["PROPERTY_VALUES"][$intHeightExtPropertyID] as $key => $val) {
//						$strNewHeightValue = $arHeight10Converter[$val["VALUE"]];
//					}
//
//					// устанавливаем новую высоту для фильтра
//					foreach($arFields["PROPERTY_VALUES"][$intHeightPropertyID] as $key => $val) {
//						$arFields["PROPERTY_VALUES"][$intHeightPropertyID][$key]["VALUE"] = $strNewHeightValue;
//					}
			...
			else
			{
				...
				elseif(strlen($save_and_add) > 0)
				{
					...
					{
						while(ob_end_clean());
						$strLink = CIBlock::GetAdminElementEditLink($IBLOCK_ID, 0, array_merge($params, array(
						"from_module" => "iblock",
						"bxpublic" => "Y",
						"nobuttons" => "Y",
					)), "&".$tabControl->ActiveTabParam());
						$strLink = str_replace($strCurrentFile, $strOriginalFilename, $strLink);
						
						$l = CUtil::JSEscape($strLink);
						?>
						<script type="text/javascript">
							top.BX.ajax.get(
					...
					{
						$strLink = $selfFolderUrl.CIBlock::GetAdminElementEditLink($IBLOCK_ID, 0, $params, "&".
								$tabControl->ActiveTabParam());
						$strLink = str_replace($strCurrentFile, $strOriginalFilename, $strLink);
						
						$saveAndAddUrl = $strLink;
				...
				{
					$strLink = $selfFolderUrl.CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array(
							"WF" => ($WF=="Y"? "Y": null),
							"find_section_section" => $find_section_section,
							"return_url" => (strlen($return_url) > 0 ? $return_url: null),
...
else
{
	...
	if ( !$bAutocomplete && !$bPropertyAjax )
	{
		...
		if (!$bCopy && $ID > 0)
		{
			...
			$copyUrl = str_replace($strCurrentFile, $strOriginalFilename, $copyUrl);
		...
		if($ID > 0 && !$bCopy)
		{
			...
			$editButtonUrl = str_replace($strCurrentFile, $strOriginalFilename, $editButtonUrl);
			if ($adminSidePanelHelper->isPublicFrame())
			{
...
if($customFormFile):
	...
	CJSCore::Init(array('date', 'jquery'));
	...
	if(in_array(12, $arUserGroups)) {
		...
		<script>
			$(function() {
                setTimeout("obIblockElementEdit1.disableAreas()", 200);
            })
		</script>
		<?
	}
	...
	if($arTranslit["TRANSLITERATION"] == "Y")
	{
		...
		function set_linked()
		{
		    if($(".js-IS_RUSSIAN.adm-designed-checkbox").is(":checked")) {
                var from = document.getElementById('NAME');
            } else {
                var from = document.getElementById('PROP_NAME_LAT');
            }
	...
	<script type="text/javascript">
		...
		var arPropertyTree = <?=CUtil::PhpToJSObject($arPropertyTree)?>;
		var arGroups = <?=CUtil::PhpToJSObject($arGroups)?>;
		var arPropertyGroups = <?=CUtil::PhpToJSObject($arPropertyGroups)?>;
	</script>
<?
...
$strLink = $selfFolderUrl.CIBlock::GetAdminElementEditLink($IBLOCK_ID, null, $arEditLinkParams);
$strLink = str_replace($strCurrentFile, $strOriginalFilename, $strLink);
...
if($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	?>
		<tr id="tr_NAME">
			...
			<td style="white-space: nowrap;">
				<input type="text" size="50" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><br>
				<a class="is_view_addictable__not_view" onclick="return obHelper.transliterate('NAME', 'PROP_NAME_LAT', 'ru2en');" href="#">Сформировать латинское название</a><?
				if(strlen($str_IBLOCK_SECTION_ID)>0 && false) {
					$arTmp = array();
					$rsNav = CIBlockSection::GetNavChain($_REQUEST["IBLOCK_ID"], $str_IBLOCK_SECTION_ID, array("NAME", "DEPTH_LEVEL"));
				...
				<a class="is_view_addictable__view" onclick="return obIblockElementEdit1.setSortDefaultName();" href="#">Установить название по умолчанию</a>
			</td>
		</tr>
	<?
	...
	?>
		<tr id="tr_CODE" class="is_view_addictable__not_view">
			<td><?echo $tabControl->GetCustomLabelHTML()?></td>
			<td style="white-space: nowrap;">
				<input type="text" size="50" name="CODE" id="CODE" maxlength="255" value="<?echo $str_CODE?>"><img id="code_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/bitrix/themes/.default/icons/iblock/link.gif" onclick="set_linked()" />
			</td>
		</tr>
	<?
...
if(!empty($PROP)):
	...
	$arViewCodes = array("IS_RUSSIAN", "SYNONYM_RUS", "NAME_LAT", "SYNONYM_LAT");
	foreach($PROP as $prop_code=>$prop_fields):
		...
		$arTrClass = array("js-tr-".str_replace("_COMMENT", '', $prop_fields["CODE"]));
		if($arPropertyGroups[str_replace("_COMMENT", '', $prop_fields["CODE"])])
			$arTrClass[] = 'js-tr-group-addictable';
		if($prop_fields["PROPERTY_TYPE"]=="F")
			$arTrClass[] = 'adm-detail-file-row';
		
		
		if(in_array(str_replace("_COMMENT", '', $prop_fields["CODE"]), $arViewCodes))
			$arTrClass[] = 'is_view_addictable__not_view';
		?>
		<tr data-code="<?=str_replace("_COMMENT", '', $prop_fields["CODE"])?>" class="<?=implode(' ', $arTrClass)?>" id="tr_PROPERTY_<?echo $prop_fields["ID"];?>">
			...
			<td width="60%"><?
				if(
					$prop_fields["USER_TYPE_SETTINGS"]["TABLE_NAME"] == 'ig_propertyvalues'
						&&
					$prop_fields["MULTIPLE"] == 'Y'
				) {
					$prop_fields["USER_TYPE_SETTINGS"]["multiple"] = 'Y';
					$prop_fields["USER_TYPE_SETTINGS"]["size"] = 10;
				}
				
				if($prop_fields["CODE"] == 'PRIORITY') {
					$strValueID = 'n0';
					$strValue = '';
					foreach($prop_fields["VALUE"] as $intValueID => $intValue) {
						$strValueID = $intValueID;
						$strValue = $intValue;
					}
					
					$strTmp = '<select class="js-PRIORITY" name="PROP['.$prop_fields["ID"].']['.$strValueID.']"><option value="">Не указано</option>';
					for($i=1;$i<11;$i++) {
						$strTmp .= '<option'.($i==$strValue?' selected':'').' value="'.$i.'">'.$i.'</option>';
					}
					$strTmp .= '</select>';
					
					echo $strTmp;
					
				} elseif(strpos($prop_fields["CODE"], "COLOR_") === 0) {
					$intNewValue = 0;
					
					// prepare values
					$arTmpValues = array();
					foreach($prop_fields["VALUE"] as $arPropValue) {
						if(isset($arPropValue["VALUE"]))
							$arTmpValues[] = $arPropValue["VALUE"];
						else $arTmpValues[] = $arPropValue;
					}
					
					foreach($arColors as $strXmlID => $arColor) {
						if($prop_fields["CODE"] != 'COLOR_LEAF' && $strXmlID == 'drK4dD37')
							continue;
						
						if(is_array($prop_fields["VALUE"])) {
							$intValueIndex = array_search($strXmlID, $arTmpValues);
						} else $intValueIndex = false;
						
						echo '<div class="color-container"><div class="color-preview" style="background-color: #'.$strXmlID.';"></div><input class="line-checkbox checkbox-color" type="checkbox"'.($intValueIndex===false?'':' checked').' value="'.$strXmlID.'" name="PROP['.$prop_fields["ID"].']['.($intValueIndex===false?"n".$intNewValue:$intValueIndex).']"> '.$arColor["NAME"].' </div>';
						
						if($intValueIndex === false) {
							$intNewValue++;
						}
					}
				} elseif($prop_fields["CODE"] == 'FLOWERING' || $prop_fields["CODE"] == 'RIPENING') {
					$arTmpValues = array();
					foreach($prop_fields["VALUE"] as $arValue) {
						if(isset($arValue["VALUE"]))
							$arTmpValues[] = $arValue["VALUE"];
						else $arTmpValues[] = $arValue;
					}
					
					$intNewValue = 0;
					for($i=1;$i<13;$i++) {
						$intValueIndex = array_search($i, $arTmpValues);
						
						echo '<input class="line-checkbox" type="checkbox"'.($intValueIndex===false?'':' checked').' value="'.$i.'" name="PROP['.$prop_fields["ID"].']['.($intValueIndex===false?"n".$intNewValue:$intValueIndex).']"> '.$i.' ';
					...
					$strPropHtml = ob_get_contents();
					ob_end_clean();
					
					
					if($prop_fields["PROPERTY_TYPE"] != 'F') {
						if($prop_fields["MULTIPLE"] == 'N') {
							if(strpos($strPropHtml, 'checkbox') === false) {
								$strPropHtml = str_replace('<input', '<input id="PROP_'.$prop_fields["CODE"].'"', $strPropHtml);
							}
							$strPropHtml = str_replace('<select', '<select id="PROP_'.$prop_fields["CODE"].'"', $strPropHtml);
						}
						
						$arDataProp = array();
						$arClass = array("js-".$prop_fields["CODE"]);
						if(isset($arPropertyTree[$prop_fields["CODE"]])) {
							$arClass[] = 'js-group-addictable';
							$arDataProp[] = 'data-code="'.$prop_fields["CODE"].'"';
						}
						
						if($prop_fields["CODE"] == 'HEIGHT_10' || $prop_fields["CODE"] == 'HEIGHT_10_EXT') {
							$arClass[] = 'js-uni-values';
						}
						
						
						$strPropHtml = str_replace('<input', '<input '.implode(" ", $arDataProp).' class="'.implode(" ", $arClass).'"', $strPropHtml);
						$strPropHtml = str_replace('<select', '<select '.implode(" ", $arDataProp).' class="'.implode(" ", $arClass).'"', $strPropHtml);
						
						
						if($prop_fields["CODE"] == 'NAME_LAT') {
							$strPropHtml .= '<a onclick="return obHelper.transliterate(\'PROP_NAME_LAT\', \'NAME\', \'en2ru\');" href="#">Сформировать русское название</a>';
						}
					}
		...
		<tr class="heading">
			<td colspan="2">
				...
				<?if (strlen($arDocumentState["ID"]) > 0 && strlen($arDocumentState["WORKFLOW_STATUS"]) > 0):
					...
					$strLink = str_replace($strCurrentFile, $strOriginalFilename, $strLink);
				?>
					(<a href="<?echo htmlspecialcharsbx($strLink)?>"><?echo GetMessage("IBEL_BIZPROC_STOP")?></a>)
				<?endif;?>
			</td>
		</tr>
		<tr>
...
elseif(!$bPropertyAjax && (!isset($_REQUEST['nobuttons']) || $_REQUEST['nobuttons'] !== "Y")):
	...
	if (!empty($arMainCatalog))
		$strLink = $selfFolderUrl.CIBlock::GetAdminElementEditLink(
				$IBLOCK_ID,
				$ID,
				$editInPanelParams
			);
...
)
{
	...
	$strLink = $selfFolderUrl.CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array("WF" => ($WF=="Y"? "Y": null), "find_section_section" => $find_section_section, "return_url" => (strlen($return_url)>0? $return_url: null)));
	$strLink = str_replace($strCurrentFile, $strOriginalFilename, $strLink);
	
	echo
		BeginNote(),
		GetMessage("IBEL_E_IBLOCK_MANAGE_HINT"),
		' <a href="'.$selfFolderUrl.'iblock_edit.php?type='.htmlspecialcharsbx($type).'&amp;lang='.LANGUAGE_ID.'&amp;ID='.$IBLOCK_ID.'&amp;admin=Y&amp;return_url='.urlencode($strLink).'">',
