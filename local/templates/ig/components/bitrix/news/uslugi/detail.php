...
if($_REQUEST["frmOrderServiceSent"] == 'Y') {
	$arErrors = array();
	...
	$intServiceID = intval($arFormData["SERVICE_ID"]);
	if($intServiceID>0) {
		if(\Bitrix\Main\Loader::includeModule("iblock")) {
			...
			if ($arI = $rsI->GetNext()) {
				$arFormData["SERVICE"] = '<a href="https://'.SITE_SERVER_NAME.$arI["DETAIL_PAGE_URL"].'">'.$arI["NAME"].'</a>';
				$arFormData["SERVICE_NAME"] = $arI["NAME"];
			}
		}
	}
	
	if(empty($arFormData["SERVICE"])) {
		$arErrors[] = '&mdash; услуга не найдена';
	}
