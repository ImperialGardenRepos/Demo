		...
		if(preg_match("/^(.*) \\[(\\d+)\\]/", $token, $match))
		{
			...
			if (0 < $match[2])
				...
				if($_REQUEST["IBLOCK_ID"] == 2) {
					...
					if($arI = $rsI -> Fetch()) {
						$arNavPath = array();
						$rsNav = \CIBlockSection::GetNavChain(\ig\CHelper::getIblockIdByCode('catalog'), $arI["IBLOCK_SECTION_ID"]);
						while ($arNav = $rsNav->Fetch()) {
							$arNavPath[] = $arNav["NAME"];
						}
					}
					$strName = implode(' / ', $arNavPath).' / '.$match[1];
				} else $strName = $match[1];
