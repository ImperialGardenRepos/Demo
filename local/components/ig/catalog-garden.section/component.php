...
$arSearchParams = \ig\CRegistry::get('catalogGroupFilter');
...
if ($this->startResultCache(false, [($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arSearchParams, $pagerParameters])) {
    ...
    if ($arParams["PRODUCT_ID"] > 0) {
        ...
        if ($arParams["IS_NEW"]) {
            ...
            $arSearchParams["PROPERTY_NEW"] = \ig\CHelper::getEnumID(\ig\CHelper::getIblockIdByCode('offers-garden'), "NEW", "Да");
    ...
    $rsI = CIBlockElement::GetList(["CATALOG_PRICE_" . \ig\CRegistry::get("CATALOG_BASE_PRICE_ID") => "ASC", "NAME" => "ASC"], $arProductFilter, false, $arNavParams, [
        "*", "PROPERTIES_*",
    ]);
    while ($obI = $rsI->GetNextElement()) {
        ...
        // get offers
        $rsO = CIBlockElement::GetList(["CATALOG_PRICE_" . \ig\CRegistry::get("CATALOG_BASE_PRICE_ID") => "ASC"], array_merge($arSearchParams, [
            "ACTIVE" => "Y",
        ...
        while ($obO = $rsO->GetNextElement()) {
            ...
            if ($arParams["PRODUCT_ID"] > 0) {
                ...
                if (\ig\CHelper::isAvailable($arO["PROPERTIES"]["AVAILABLE"]["VALUE"])) {
    ...
    if (!empty($arResult["SECTIONS"])) {
        ...
        while ($arSec = $rsSec->GetNext()) {
            $rsNav = CIBlockSection::GetNavChain(\ig\CHelper::getIblockIdByCode('catalog'), $arSec["ID"], ["NAME", "SECTION_PAGE_URL", "ID"]);
    ...
    if (!empty($arResult["IBLOCK_SECTIONS"])) {
        $rsSecTmp = \CIBlockSection::GetList([], ["IBLOCK_ID" => \ig\CHelper::getIblockIdByCode('catalog'), "ID" => array_keys($arResult["IBLOCK_SECTIONS"])], false, ["NAME", "SECTION_PAGE_URL", "UF_NAME_LAT"]);
...
if ($arParams["IS_AJAX"]) {
    if ($arParams["PAGE_NUM"] > 0 || $arParams["PRODUCT_ID"] > 0) {
        ...
        \ig\CRegistry::add('catalog-results_html', $strContents);
