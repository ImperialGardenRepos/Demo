...
if ($obCache->initCache($arParams["CACHE_TIME"], $strCacheID)) {
    ...
    while ($arList = $rsList->Fetch()) {
        $arResult["OFFER_PARAMS"]["CATALOG_PRICE_LIST"]["VALUES"][$arList["UF_XML_ID"]] = array(
            ...
            "SPHINX_VALUE" => CatalogGardenOffers::convertValueToSphinx("CATALOG_PRICE_LIST", $arList["UF_XML_ID"]),
            "COUNT" => 0,
            "DISABLED" => "N"
        );
    }
    ...
    $rsList = CIBlockPropertyEnum::GetList(Array(
        "DEF" => "DESC",
        "SORT" => "ASC"
    ), Array("IBLOCK_ID" => CHelper::getIblockIdByCode('offers-garden'), "CODE" => "AVAILABLE"));
    while ($arList = $rsList->Fetch()) {
        $arResult["OFFER_PARAMS"]["AVAILABLE"]["VALUES"][$arList["ID"]] = array(
            "ID" => $arList["ID"],
            "NAME" => $arList["VALUE"],
            "VALUE" => $arList["ID"],
            "SPHINX_VALUE" => CatalogGardenOffers::convertValueToSphinx("PROPERTY_AVAILABLE", $arList["ID"]),
            "COUNT" => 0,
            "DISABLED" => "N"
        );
    }
    ...
    $rsList = CIBlockPropertyEnum::GetList(Array(
        "DEF" => "DESC",
        "SORT" => "ASC"
    ), Array("IBLOCK_ID" => CHelper::getIblockIdByCode('catalog-garden'), "CODE" => "USAGE"));
    while ($arList = $rsList->Fetch()) {
        $arResult["OFFER_PARAMS"]["USAGE"]["VALUES"][$arList["ID"]] = array(
            "ID" => $arList["ID"],
            "NAME" => $arList["VALUE"],
            "VALUE" => $arList["ID"],
            "SPHINX_VALUE" => CatalogGardenOffers::convertValueToSphinx("PROPERTY_USAGE", $arList["ID"]),
            "COUNT" => 0,
            "DISABLED" => "N"
        );
    }
...
if ($_REQUEST["search"] == 'Y') {
    ...
    if ($_REQUEST["frmCatalogFilterSent"] == 'Y') {
        ...
        foreach ($_REQUEST["F"] as $strCode => $obValue) {
            ...
            if (!empty(CatalogFilter::$arRequestFilter[$strCode])) {
                $arSearchParams[$strCode] = CatalogGardenOffers::convertValueToSphinx($strCode, CatalogFilter::$arRequestFilter[$strCode]);
            }
        }
    ...
    if ($arResult["IS_ACTION"]) {
        ...
        $intNewEnumID = CHelper::getEnumID(CHelper::getIblockIdByCode('offers-garden'), "NEW", "Да");
...
// фасетим по нефильтрованным параметрам
        $arFacetData = $obSearch->searchFacet(array("FILTER" => $arSearchParams), CatalogGardenOffers::getFacetFields(array("EXCLUDE" => $arFacetExcludeParams)), true);
        foreach ($arFacetData["FACET"] as $strSphinxCode => $arDataList) {
            $strCode = str_replace("PROPERTY_", '', CatalogGardenOffers::convertFieldCode($strSphinxCode, false));

            $arTotalFilter = $arSearchParams;
            unset($arTotalFilter[CatalogGardenOffers::convertFieldCode($strSphinxCode, false)]);
...
// фасетим по фильтрованным параметрам
        foreach ($arSearchParams as $strCode => $value) {
            ...
            $arParamData = CatalogGardenOffers::getSphinxConfig(CatalogGardenOffers::convertFieldCode($strCode));
            ...
            if ($arParamData["CONTROL"] == 'CHECKBOX' && $arParamData["MULTIPLE"] == 'Y') {
                ...
                $arFacetData = $obSearch->searchFacet(array("FILTER" => $arTmpFilter), CatalogGardenOffers::getFacetFields(array("INCLUDE" => $strCode)), true);

                foreach ($arFacetData["FACET"] as $strSphinxCode => $arDataList) {
                    $strCodeTmp = str_replace("PROPERTY_", '', CatalogGardenOffers::convertFieldCode($strSphinxCode, false));
                    foreach ($arDataList as $arData) {
    ...
    if ($intSelectedSectionID > 0) {
        $rsSec = CIBlockSection::GetList(Array(), array("ACTIVE" => "Y", "IBLOCK_ID" => CHelper::getIblockIdByCode('catalog-garden'), "ID" => $intSelectedSectionID), false, array("ID", "CODE", "IBLOCK_ID", "SECTION_PAGE_URL"));
    ...
    CRegistry::add('catalogGroupFilter', $arSearchParams);
