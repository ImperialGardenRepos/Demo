...
//							$floatMinPrice = (empty($arBasketItem["DISCOUNT_PRICE"])?$arBasketItem["BASE_PRICE"]:$arBasketItem["DISCOUNT_PRICE"]);
															...
															<div class="ptgb__content">
																...
																<div class="ptgb__title">
																	<div class="js-icard-price-discount-wrapper<?=($floatBasePrice>0?'':' hidden')?>">
																		...
																		<div class="ccard__price-old"><span class="line-through js-icard-price"><?=\ig\CFormat::getFormattedPrice($floatBasePrice, "RUB", array("RUB_SIGN"=>''))?></span></div>
																	</div>
																	<div class="js-icard-price-wrapper<?=($floatBasePrice>0?' hidden':'')?>">
																		<div class="ccard__price"><span class="font-bold js-icard-price"><?=\ig\CFormat::getFormattedPrice($floatMinPrice, "RUB", array("RUB_SIGN"=>''))?></span> <span class="font-light">₽</span></div>
																	</div>
																</div>
																...
																if(false) { ?>
																<div class="ptgb__title">
																	<div class="ccard__price<?=(isset($floatBasePrice) ? ' color-active' : '')?>"><span class="font-bold js-icard-price"><?=\ig\CFormat::getFormattedPrice($floatMinPrice, "RUB", array("RUB_SIGN"=>''))?></span> <span class="font-light">₽</span></div>
																	<?
																	if(isset($floatBasePrice)) {?>
																		<div class="ccard__price-old"><span class="line-through"><?=\ig\CFormat::getFormattedPrice($floatBasePrice, "RUB", array("RUB_SIGN"=>''))?></span></div><?
																	}?>
															...
															<div class="ptgb__content">
																...
																<div class="ptgb__title ptgb__title--textfield">
						                                            <span class="input-spin touch-focused" data-trigger="spinner">
						                                                ...
						                                                <input type="tel" class="input-spin__textfield textfield keyfilter-pint js-icard-spinner" data-spin="spinner" name="QUANTITY[<?=$arOffer["ID"]?>]" data-rule="quantity" data-min="0" data-max="9999" data-step="1" value="<?=$arBasketItem["QUANTITY"]?>" maxlength="4" size="6">
				...
				<div class="form__items">
								...
								<div class="col">
									<div class="form__item-field form__item-field--error-absolute">
										<div class="textfield-wrapper">
											<input type="text" class="textfield" name="ORDER_PROP_<?=$arOrderProp["ID"]?>"<?=($arOrderProp["REQUIRED"] == 'Y'?' data-rule-required="true"':'').($arOrderProp["IS_EMAIL"]=='Y'?' data-rule-email="true"':'')?> value="<?=$arOrderProp["VALUE"]?>">
					...
					<div class="form__item">
						<div class="form__item-field form__item-field--error-absolute">
							<div class="textfield-wrapper">
								<input type="text" class="textfield" name="ORDER_PROP_<?=$arOrderProp["ID"]?>" value="<?=$arOrderProp["VALUE"]?>"<?=($arOrderProp["REQUIRED"] == 'Y'?' data-rule-required="true"':'')?>>
								<div class="textfield-placeholder"><?=$arOrderProp["NAME"]?></div><?
								if($arOrderProp["REQUIRED"] == 'Y') {?>
									<div class="textfield-after color-active">*</div><?
								}?>
					...
					<div class="form__item">
						<div class="form__item-field form__item-field--error-absolute">
							<div class="textfield-wrapper">
								<input type="text" class="textfield" value="<?=$arResult["USER_VALS"]["ORDER_DESCRIPTION"]?>" name="ORDER_DESCRIPTION">
						...
						<div class="checkboxes checkboxes--w-title">
							<div class="checkboxes__inner"><?
								...
								<label class="checkbox checkbox--w-title checkbox--radio checkbox--block checkbox-plain-js">
									...
									<input type="radio" name="<?=$arDelivery["FIELD_NAME"]?>" value="<?=$arDelivery["ID"]?>"<?=($arDelivery["ID"]==$arResult["USER_VALS"]["DELIVERY_ID"]?' checked':'')?>>
									<div>
								...
								<label class="checkbox checkbox--w-title checkbox--radio checkbox--block checkbox-plain-js">
									...
									<div>
										...
										<div class="checkbox__summary">
											<p>Вы можете самостоятельно забрать свой заказ после того, как с Вами свяжется менеджер и подтвердит наличие и готовность заказа к выдаченна территории садового
												центра IG.</p>
											
											<p>Пожалуйста, прежде чем приежать за заказом согласуйте время и дату самовывоза - дождитесь звонка нашего менеджера или посвоните сами по телефону:</p>
					...
					<div class="cols-wrapper">
						<div class="cols">
								...
								<div class="checkboxes">
									<div class="checkboxes__inner">
										<label class="checkbox checkbox--block checkbox-plain-js">
											<div class="checkbox__icon"></div>
											<div class="form__item-field form__item-field--error-absolute">
												<input type="checkbox" class="checkbox__input" name="OFERTA" value="Y" checked data-rule-required="true" data-msg-required="Вам нужно согласиться с условиями оферты.">
												Я ознакомлен с условиями и согласен с <a target="_blank" href="/download/politika.pdf">политикой конфиденциальности</a>
											</div>
										</label><?
										$arOrderProp = $arPropTmp["SUBSCRIBE"];?>
										<label class="checkbox checkbox--block checkbox-plain-js">
											<div class="checkbox__icon"></div>
											<input type="checkbox" name="ORDER_PROP_<?=$arOrderProp["ID"]?>"<?=($arResult["USER_VALS"]["ORDER_PROP"][$arOrderProp["ID"]]=='Y'?' checked':'')?> value="Y">
							...
							<div class="col text-align-right text-align-center-on-mobile">
								<button type="submit" name="confirmorder" value="Y" class="btn btn--large">Отправить заказ</button>
							</div>
						</div>
					</div>
