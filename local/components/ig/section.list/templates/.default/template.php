...
?>
    ...
    <div class="section">
        <div class="container">
            <h1 class="h1 text-align-center"><?php $APPLICATION->ShowTitle() ?></h1>
        </div>
    </div>
<?
if ($strNav) { ?>
...
foreach ($arResult["SECTIONS"] as $arSection) {
    ...
    ?>
<div class="section section--block bg-pattern <?= $arBGClass[$intCnt] ?> section--saw-before"
     id="<?= $arSection["CODE"] ?>" data-goto-offset-element=".header">
    <div class="container">
                ...
                foreach ($arResult["ITEMS"][$arSection["ID"]] as $arItem) { ?>
                    ...
                    <a class="tgb__inner" href="<?= $arItem["DETAIL_PAGE_URL"] ?>">
                        <div class="tgb__image img-to-bg">
                            <img data-lazy-src="<?= $arItem["TILE"]["src"] ?>"
                                 src="<?= SITE_TEMPLATE_PATH ?>/img/blank.png" alt="<?= $arItem["NAME"] ?>">
                        </div>
