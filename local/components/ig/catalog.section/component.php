...
$arSearchParams = \ig\CRegistry::get('catalogFilter');
...
//				$arPath["IPROPERTY_VALUES"] = $ipropValues->getValues();
	...
	$arFilterExt = array();
	$arOffersFilterExt = array();
	
	$strCurrentDir = $APPLICATION->GetcurDir();
	if($arParams["IS_NEW"]) {
		$arSearchParams["PROPERTY_NEW"] = \ig\CHelper::getEnumID(\ig\CHelper::getIblockIdByCode('offers'), "NEW", "Да");
	...
	unset($arOffersSearchParams["FILTER"]["vidsID"]);
	unset($arOffersSearchParams["FILTER"]["sortsID"]);
	...
	$rsI = CIBlockElement::GetList(Array("CATALOG_PRICE_".\ig\CRegistry::get("CATALOG_BASE_PRICE_ID") => "ASC", "NAME" => "ASC"), $arSortFilter, false, $arNavParams, array(
		"*", "PROPERTIES_*"
	));
	...
	if(!empty($arResult["ITEMS"])) {
		// get offers
		$rsO = CIBlockElement::GetList(Array("CATALOG_PRICE_".\ig\CRegistry::get("CATALOG_BASE_PRICE_ID") => "ASC"), array_merge($arSearchParams, array(
			"ACTIVE"             => "Y",
			"IBLOCK_ID"          => \ig\CHelper::getIblockIdByCode('offers'),
			"PROPERTY_CML2_LINK" => array_keys($arResult["ITEMS"])
		), $arOffersFilterExt), false, false, $arOffersSelect);
		...
		while ($obO = $rsO->GetNextElement()) {
			...
			if($arParams["SORT_ID"]>0) {
				...
				if(\ig\CHelper::isAvailable($arO["PROPERTY_AVAILABLE_VALUE"])) {
