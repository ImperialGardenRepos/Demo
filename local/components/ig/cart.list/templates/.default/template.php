...
if (empty($arResult["ITEMS"])) { ?>
        ...
        <div class="icards icards--compact">
            <div class="icards__inner"><?
                foreach ($arResult["ITEMS"] as $arOffer) {
                    ...
                    if ($arOffer["IBLOCK_ID"] == CHelper::getIblockIdByCode('offers')) {
                    ...
                    if ($arSort["IBLOCK_ID"] == CHelper::getIblockIdByCode('catalog')) {
                        $strFormattedName = CFormat::formatPlantTitle((empty($arSortProp["IS_VIEW"]["VALUE"]) ? $strName : ''), $arSectionNav[0]["NAME"], $arSectionNav[1]["NAME"]);
                    } else {
                    ...
                    ?>
                    <div class="icard icard--compact js-icard-use-discounts js-icard js-icard-to-remove"
                         data-offer-index="0" data-offers-url="ajax/icard-offers.php">
                    <div class="icard__inner">
                        <div class="icard__content">
                            <div class="icard__header">
                                <div class="icard__title"><a href="<?= $arSort["DETAIL_PAGE_URL"] ?>"
                                                             class="link--bordered-pseudo"><?= $strFormattedName ?></a>
                                </div><?
                                ...
                                <div class="icard__main-grid"><?
                                    if ($arImage["RESIZE"]["src"]) { ?>
                                        ...
                                        <div class="icard__image-inner js-icard-offer-images">
                                            <div class="icard__image-item image-slider active image-slider--noshadow">
                                                <img class="active" data-lazy-src="<?= $arImage["RESIZE"]["src"] ?>"
                                                     src="<?= SITE_TEMPLATE_PATH ?>/img/blank.png"
                                                     data-src-full="<?= $arImage["SRC"] ?>"
                                                     data-ihs-content="<?= $strName ?>" alt="<?= $strName ?>">
                                            </div>
                                        </div>
                                        </div><?
                                    }
                                    ...
                                    if ($arSort["IBLOCK_ID"] == CHelper::getIblockIdByCode('catalog')) { ?>
                                        ...
                                        <div class="ptgbs ptgbs--features">
                                            <?= CFormat::formatListMainProperties($arSort); ?>
                                        </div>
                                        </div><?
                                    } ?>
    ...
    <div class="header__popover-footer">
        ...
        <div class="header__popover-footer-inner">
            <div class="header__popover-message text-align-center">Товаров в корзине: <?= count($arResult["ITEMS"]) ?>.
                На сумму:
                <strong><?= CFormat::getFormattedPrice($arResult["CART_SUMM"], "RUB", array("RUB_SIGN" => "₽")) ?></strong>
            </div>
        </div>
